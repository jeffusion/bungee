...
| **🎨 Web 仪表板** | 现代化、响应式的 Svelte + Tailwind + shadcn/Bits 工业风 UI，用于实时监控、路由管理和配置，带有可视化图表和指标。 |
...
| **🔀 API 格式转换** | 内置转换器，实现无缝 API 兼容性（如 `anthropic-to-gemini`、`openai-to-anthropic`）。另提供 [`openai-messages-to-chat` 兼容指南](docs/openai-messages-to-chat.md)。 |
| **🔌 插件系统** | 可扩展的插件架构，支持 Hooks、自定义 API 和原生仪表板组件。详见[插件系统](docs/plugin-system.md)。 |
...
```json
{
  ...
  "routes": [{ "path": "/api/gemini", "service": "gemini-api" }]
}
```
...
- **⚙️ 配置管理**：
  ...
  - 实时配置验证反馈
- **💡 用户友好界面**：
  - 使用 Svelte、Tailwind 和 shadcn/Bits 工业风 UI 构建
  - 响应式设计，适配桌面和移动设备
  - 支持暗色模式
  - 所有操作都有 Toast 通知
...
```json
{
  ...
  "workers": 2,                 // 工作进程数量
  "port": 8088,                 // 服务器端口
  "body_parser_limit": "50mb",  // 最大请求体大小
  "services": [...],            // 可复用服务定义
  "routes": [...]               // 路由配置
}
```
...
- `path`：匹配的 URL 路径前缀
- `path_rewrite`：（可选）用于重写路径的正则表达式模式
- `service`：引用 `services` 中的服务名称
- `endpoints`：（可选）仅在特殊场景内联定义端点
- `headers`、`body`：（可选）路由级修改规则
- `plugins`：（可选）用于转换的插件名称数组
- `failover`、`health_check`：（可选）高可用性配置
...
- `retry_on` 支持表达式语法（如 `>=400`、`!404`、`4xx`、`5xx`），也可以用逗号组合：`>=500,!503`。
- `auto_disable_threshold` 用于在连续失败达到阈值时自动禁用端点，防止继续拉低集群。
- `auto_enable_on_active_health_check` 会在主动健康检查判定健康后自动重新启用被禁用的端点。
