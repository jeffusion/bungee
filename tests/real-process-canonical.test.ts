...
type DaemonHarness = Readonly<{
  ...
  diagnoseExit(child: ChildProcess, role: 'A.second' | 'C.single', preStopExited: boolean): Promise<void>;
}>;
...
async function reservePortBlock(): Promise<PortLease> {
  for (;;) {
    ...
    try {
      ...
      fourth = Bun.serve({ hostname: '127.0.0.1', port: base + 3, fetch: () => new Response('reserved') });
      ...
      await Promise.allSettled([first.stop(true), second?.stop(true), third?.stop(true), fourth?.stop(true)].filter((value): value is Promise<void> => value !== undefined));
...
async function makeFixture(root: string, name: string, credentialPort: number): Promise<Fixture> {
  ...
  const controlAuditPath = join(fixtureRoot, 'control-rpc.log');
  const tlsCertPath = join(root, 'tls', 'cert.pem');
  ...
  await writeFile(join(pluginPath, 'manifest.json'), JSON.stringify({
    ...
    ], upstreamSources: [{ id: 'canonical', label: 'Canonical', listAccounts: 'listAccounts', createDraft: 'createDraft', credentialPolicy: {
...
async function createTlsUpstream(root: string, marker: () => string): Promise<ReturnType<typeof Bun.serve>> {
  ...
  return Bun.serve({
    ...
    fetch: (request) => request.headers.get('x-canonical-token') === 'canonical'
      ? new Response(marker(), { status: 200 })
      : new Response('unauthorized', { status: 401 }),
  });
}

async function assertTlsRequiresTrust(url: string): Promise<void> {
  const environment = withoutTlsEnvironment();
  const child = spawn(process.execPath, ['-e', `fetch(process.argv.at(-1)).then(() => console.log('TLS_UNEXPECTED_SUCCESS')).catch((error) => console.log(error instanceof Error && /certificate|tls|self-signed|verify/i.test(error.message) ? 'TLS_UNTRUSTED' : 'TLS_OTHER_ERROR'), process.argv[1])`, url], {
    env: environment, stdio: ['ignore', 'pipe', 'ignore'],
  });
  let output = '';
  child.stdout?.on('data', (chunk: Buffer) => { output += chunk.toString('utf8'); });
  await childExit(child);
  expect(output.trim()).toBe('TLS_UNTRUSTED');
}

function withoutTlsEnvironment(): NodeJS.ProcessEnv {
...
async function cleanupUnownedCoreSetup(
  ...
  upstream: ReturnType<typeof Bun.serve> | undefined,
  managedUpstream?: ReturnType<typeof Bun.serve>,
): Promise<unknown[]> {
  const errors: unknown[] = [];
  try { if (upstream !== undefined) await upstream.stop(true); } catch (error) { errors.push(error); }
  try { if (managedUpstream !== undefined) await managedUpstream.stop(true); } catch (error) { errors.push(error); }
  if (lease !== undefined) {
    try { await closePorts(lease); } catch (error) { quarantineTestPortBlock(lease.block); errors.push(error); }
    try {
      if (!releaseTestPortBlock(lease.block)) errors.push(new Error('canonical core setup port lease was not active'));
...
async function createUnownedCoreSetup(
  root: string,
  reserve: () => Promise<PortLease> = reservePortBlock,
  marker: () => string = () => 'A',
): Promise<Readonly<{
  lease: PortLease;
  ...
  let managedUpstream: ReturnType<typeof Bun.serve> | undefined;
  try {
    lease = await reserve();
    upstream = Bun.serve({ hostname: '127.0.0.1', port: 0, fetch: () => new Response(marker()) });
    if (upstream.port === undefined) throw new Error('upstream did not bind');
    managedUpstream = await createTlsUpstream(root, marker);
    if (managedUpstream.port === undefined) throw new Error('managed upstream did not bind');
    const fixture = await makeFixture(root, 'core', managedUpstream.port);
    const daemon = await createDaemonHarness(fixture.root, lease, fixture, 1, {
      ...
      tlsCertPath: fixture.tlsCertPath,
    });
    return { lease, upstream, managedUpstream, fixture, daemon };
  } catch (error) {
    const cleanupErrors = await cleanupUnownedCoreSetup(root, lease, upstream, managedUpstream);
    if (cleanupErrors.length > 0) throw new AggregateError([error, ...cleanupErrors], 'canonical core setup failed');
    throw error;
  }
}

function coreEnvironment(fixture: Fixture, lease: PortLease, workers = 2, tlsCertPath?: string): NodeJS.ProcessEnv {
  const inherited = withoutTlsEnvironment();
  return {
    ...inherited,
    ...
    ...(tlsCertPath === undefined ? {} : { NODE_EXTRA_CA_CERTS: tlsCertPath }),
...
async function createDaemonHarness(root: string, lease: PortLease, fixture: Fixture | undefined, workers = 2, options: Readonly<{
  ...
  const manager = new DaemonManager((executable, args, spawnOptions) => {
    ...
    const appStarts = captureAppLogOffsets(logsDirectory);
    const daemonStarts = logFiles.map((file) => { try { return statSync(file).size; } catch { return 0; } });
    const child = spawn(executable, [...args], spawnOptions);
    const appFreeze = createExitAppLogFreeze(child, appStarts, logsDirectory);
    let daemonEnds: Array<number | null> | undefined;
    const captureDaemonEnds = (): void => { daemonEnds = logFiles.map((file) => { try { return statSync(file).size; } catch { return null; } }); };
    child.once('exit', captureDaemonEnds);
    if (child.exitCode !== null || child.signalCode !== null) captureDaemonEnds();
    childExitWindows.set(child, async () => {
      const exited = child.exitCode !== null || child.signalCode !== null;
      const app = exited ? appFreeze() : freezeAppLogWindows(appStarts, logsDirectory);
      const daemon = logFiles.map((file, index) => ({ file, start: daemonStarts[index] ?? 0, end: exited ? daemonEnds?.[index] ?? null : (() => { try { return statSync(file).size; } catch { return null; } })() }));
      const windows = [...app, ...daemon];
      const lengths = windows.map((window) => window.end === null ? null : window.end - window.start);
      const perWindowCap = 16 * 1024;
      const reads = await Promise.all(windows.map((window) => readExitDiagnosticWindow(window.file, window.start, window.end, perWindowCap, !('available' in window) || window.available !== false)));
      const unavailable = reads.some((read) => !read.readable);
      const truncated = lengths.some((length, index) => length !== null && (length < 0 || length > perWindowCap || !reads[index]?.complete));
      // Four diagnostic windows are each capped at 16 KiB (64 KiB total). App phase classification
...
type AppLogFreeze = Readonly<{ file: string; start: number; end: number; available?: false }>;
  ...
  return [...starts.entries()].sort(([left], [right]) => (left < right ? -1 : left > right ? 1 : 0)).slice(0, APP_LOG_MAX_FILES)
    .map(([file, start]) => {
      try {
        ...
        if (info.isFile()) return { file, start, end: info.size };
        return { file, start, end: start, available: false };
      } catch { return { file, start, end: start, available: false }; }
    });
}
...
async function readExitDiagnosticWindow(file: string, start: number, end: number | null, cap: number, available = true): Promise<Readonly<{ text: string; readable: boolean; complete: boolean }>> {
  if (!available || end === null) return { text: '', readable: false, complete: false };
  const length = end - start;
  if (length <= 0) return { text: '', readable: true, complete: length === 0 };
  const take = Math.min(length, cap);
  try {
    const bytes = await Bun.file(file).slice(start + length - take, start + length).arrayBuffer();
    return { text: new TextDecoder().decode(bytes), readable: true, complete: length <= cap && bytes.byteLength === length };
...
const RUNTIME_CODE_ALLOWLIST: ReadonlySet<string> = new Set([
  'invalid_state', 'startup_incomplete', 'startup_cancelled', 'admission_mismatch',
  'listener_port_unavailable', 'repair_failed', 'publication_failed',
  'cleanup_failed', 'worker_exit_unconfirmed',
]);
...
function aggregate(upstreamPort: number, path = '/proxy', managed = false, managedPort = upstreamPort): ConfigurationAggregateV2 {
  const endpoint = {
    id: '20000000-0000-4000-8000-000000000001', position: 1, target: `http://127.0.0.1:${upstreamPort}`, weight: 100, priority: 1, is_disabled: false, plugins: [],
  };
  const managedEndpoint = {
    id: '20000000-0000-4000-8000-000000000002', position: 1, target: `https://127.0.0.1:${managedPort}/`, weight: 100, priority: 1, is_disabled: false,
    managedBy: { plugin: 'canonical-plugin', contributionId: 'canonical', bindingId: '40000000-0000-4000-8000-000000000001' },
    plugins: [{ id: '40000000-0000-4000-8000-000000000001', position: 1, name: 'canonical-plugin', enabled: true, options: {} }],
  };
...
async function runtimeWorkers(port: number): Promise<readonly { readonly pid: number; readonly boot_nonce: string; readonly worker_instance_id: string }[]> {
  ...
  return (body.workers ?? []).map((worker) => {
    if (!Number.isSafeInteger(worker.pid) || typeof worker.boot_nonce !== 'string' || typeof worker.worker_instance_id !== 'string') {
      throw new Error(`runtime worker identity is incomplete: ${JSON.stringify(worker)}`);
    }
    return { pid: worker.pid!, boot_nonce: worker.boot_nonce, worker_instance_id: worker.worker_instance_id };
  });
}
...
describe.serial('A core lifecycle', () => {
  ...
  test('cleans the core root when port reservation fails', async () => {
    const root = makeCanonicalTempDir('bungee-canonical-core-reserve-failure', { daemonSafe: true });
    const failure = new Error('test port reservation failed');
    await expect(createUnownedCoreSetup(root, async () => { throw failure; })).rejects.toBe(failure);
    let code: string | undefined;
    try { await stat(root); } catch (error) { code = (error as NodeJS.ErrnoException).code; }
  ...
  test('starts the canonical master, separates ports, and publishes A', async () => {
    ...
    const initialManaged = await fetch(`http://127.0.0.1:${lease.base + 1}/managed-proxy`);
  ...
  test('keeps A alive after SIGKILL, rejects the concurrent owner, and rejects stale mutation', async () => {
    ...
    const adoptedManaged = await fetch(`http://127.0.0.1:${state.lease.base + 1}/managed-proxy`);
    expect(adoptedManaged.status).toBe(200);
    expect(await adoptedManaged.text()).toBe('A');
    expect(await readFile(state.fixture.controlAuditPath, 'utf8').catch(() => '')).toContain(`rpc:${state.second?.pid}`);
    state.competitor = spawnCore(state.fixture, state.lease, 1);
  ...
  test('publishes B and closes management, public, and supervision ports', async () => {
    ...
    catch (error) { await state.daemon.diagnoseExit(state.second, 'A.second', preStopExited); throw error; }
    const secondExit = await childExit(state.second);
    if (secondExit.code !== 0 || secondExit.signal !== null) await state.daemon.diagnoseExit(state.second, 'A.second', preStopExited);
...
describe('daemon exit log summary helper', () => {
  test('skips app log content, reads fixed daemon categories, and keeps live snapshots uncached', async () => {
    const summary = summarizeDaemonExitLogs([
      ...
      '2026-09-23T00:00:00.000Z \u001b[31merror\u001b[39m: Master runtime failed {"error":{"code":"repair_failed"}}',
      '{"level":"error","message":"Process startup failed","error":{"code":"not-allowed"}}',
    ], 1);
    expect(summary.messages).toEqual([['Master runtime failed'], ['Process startup failed']]);
    expect(summary.codes).toEqual(['repair_failed']);

    const root = makeCanonicalTempDir('bungee-exit-window-check', { daemonSafe: true });
    try {
      const file = join(root, 'app-test.log');
      await writeFile(file, 'first');
      const child = new EventEmitter() as ChildProcess;
      Object.defineProperties(child, { exitCode: { value: null }, signalCode: { value: null } });
      const windows = [{ file, start: 0 }];
      const frozen = createExitAppLogFreeze(child, windows, root);
      const live = freezeAppLogWindows(windows, root);
      expect(live[0]?.end).toBe(5);
      await appendFile(file, 'later');
      child.emit('exit');
      expect(frozen()[0]?.end).toBe(10);
      const short = await readExitDiagnosticWindow(file, 0, 20, 16);
      expect(short.readable).toBeTrue();
      expect(short.complete).toBeFalse();
      const missingFreeze = freezeAppLogWindows([{ file: join(root, 'missing-app.log'), start: 0 }], root);
      expect(missingFreeze.find((window) => window.file.endsWith('missing-app.log'))?.available).toBeFalse();
      const missingDaemon = await readExitDiagnosticWindow(file, 0, null, 16);
      expect(missingDaemon.readable).toBeFalse();
      const emptyFile = join(root, 'empty.log');
      await writeFile(emptyFile, '');
      const empty = await readExitDiagnosticWindow(emptyFile, 0, statSync(emptyFile).size, 16);
      expect(empty).toEqual({ text: '', readable: true, complete: true });
...
describe.serial('B daemon', () => {
  ...
  beforeAll(async () => {
    ...
    const daemon = await createDaemonHarness(root, lease, undefined, 1, {
      ...
      baseEnvironment: {
        ...withoutTlsEnvironment(), HOME: home, USERPROFILE: home,
  ...
  test('starts and rejects missing or wrong shutdown credentials', async () => {
    ...
    expect(metadata.management_host).toBe('127.0.0.1');
    expect(metadata.management_port).toBe(state.lease.block.ports[3]);
...
describe.serial('C benchmark', () => {
  ...
  beforeAll(async () => {
    ...
    const fixture = await makeFixture(root, 'benchmark', upstream.port);
  ...
  test('BA runs the same canonical proxy in B/A order and cleans up', async () => {
    ...
    const preStopExited = state.child.exitCode !== null || state.child.signalCode !== null;
    try { await state.daemon.manager.stop(); }
    catch (error) { await state.daemon.diagnoseExit(state.child, 'C.single', preStopExited); throw error; }
    const childExitResult = await childExit(state.child);
    if (childExitResult.code !== 0 || childExitResult.signal !== null) await state.daemon.diagnoseExit(state.child, 'C.single', preStopExited);
    expect(childExitResult).toEqual({ code: 0, signal: null });
