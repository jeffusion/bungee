...
test('native account page compiles and uses standard controls, no iframe SDK or persistent authorization data', async () => {
  ...
  for (const component of ['Dialog.Root', 'Dialog.Title', 'Select.Root', 'RadioGroup.Root', 'DropdownMenu.Root', 'StatusBadge', 'PanelCard', 'Input', 'Button']) expect(source).toContain(`<${component}`);
