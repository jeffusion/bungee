...
test('stopped → exact retry body → 202 running → terminal hides; duplicate click is blocked', async () => {
  ...
  expect(request.path).toBe(`/api/config/operations/${runtime.publication.operation!.operation_id}/retry`);
...
test('runtime failure invalidates serving evidence rather than retaining green health', async () => {
  const store = await start();
  expect(get(store).fresh).toBe(true);
  expect(get(store).runtime).not.toBeNull();
  globalThis.fetch = (async () => { throw new TypeError('offline'); }) as unknown as typeof fetch;
  await store.refresh();
  expect(get(store).fresh).toBe(false);
  expect(get(store).runtime).toBeNull();
});

test('a hung runtime read expires prior confirmed evidence; pause is not a read failure', async () => {
  const store = createPublicationRecoveryStore({ staleAfterMs: 25, readTimeoutMs: 50 });
  unsubscribe = store.subscribe(() => {}); await store.refresh(); expect(get(store).fresh).toBe(true);
  globalThis.fetch = (async (_input: string, init?: RequestInit) => new Promise((_resolve, reject) => {
    init?.signal?.addEventListener('abort', () => reject(new DOMException('aborted', 'AbortError')));
  })) as typeof fetch;
  const hanging = store.refresh(); await Bun.sleep(35);
