...
try {
  for (const [name, width, locale] of [['desktop', 1440, 'en'], ['mobile', 390, 'zh-CN']] as const) {
    ...
    await page.route('**/__ui/api/**', async route => {
      ...
      if (path === '/__ui/api/auth/verify') return route.fulfill({ json: { success: true } });
      if (path === '/__ui/api/config/runtime') { runtimeReads++; return route.fulfill({ json: runtime }); }
      if (path === '/__ui/api/config') return route.fulfill({ json: runtime });
      if (path.endsWith('/retry') && request.method() === 'POST') {
        ...
        assert.equal(path, `/__ui/api/config/operations/${runtime.publication.operation!.operation_id}/retry`);
      ...
      if (path === '/__ui/api/runtime/upstreams') return route.fulfill({ json: {
      ...
      if (path === '/__ui/api/plugins') return route.fulfill({ json: [] });
      if (path === '/__ui/api/plugin-translations') return route.fulfill({ json: {} });
      if (path === '/__ui/api/stats/history/v2') return route.fulfill({ json: {
        timestamps: ['2026-09-13T12:00:00Z'], requests: [120], errors: [0], responseTime: [85], successRate: [100],
      } });
      if (path === '/__ui/api/stats/upstream-stats' || path === '/__ui/api/stats/upstream-status-codes') return route.fulfill({ json: { data: [], type: 'all' } });
      errors.push(`Unexpected management request: ${path}`);
      return route.fulfill({ status: 500, json: { error: 'unexpected_mock_request' } });
    });
    await page.goto(`${base}/__ui/#/`, { waitUntil: 'networkidle' });
