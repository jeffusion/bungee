...
try {
  ...
  await page.route('**/*', async route => {
    ...
    if (url.pathname === '/__ui/plugins/sandbox-plugin/index.html') {
    ...
    if (url.pathname === '/__ui/api/plugins/sandbox-plugin/sandbox') return route.fulfill({ json: {
      sandbox: 'allow-scripts', allowedHostActions: ['ui-context', 'copy-styles', 'control'], controlAllowlist: [{ path: '/allowed', methods: ['POST'] }],
    } });
    if (url.pathname === '/__ui/api/plugins') return route.fulfill({ json: [{ name: 'sandbox-plugin', enabled: true, metadata: { contributes: { api: [{ path: '/allowed', methods: ['POST'], handler: 'allowed', execution: 'control' }] } } }] });
    if (url.pathname === '/api/plugins/sandbox-plugin/control/allowed') { controlCalls++; return route.fulfill({ json: { ok: true } }); }
    return route.continue();
  });

  const base = `http://127.0.0.1:${(server.httpServer!.address() as { port: number }).port}/__ui/tests/fixtures/sandbox.html`;
