...
export class AnthropicToGeminiConverter implements AIConverter {
  ...
  async onResponse(ctx: ResponseContext): Promise<Response | void> {
    ...
    if (!ctx.response.ok) {
      if (!this.isLikelyGeminiErrorBody(geminiBody)) {
        return ctx.response;
      }
    ...
    if (!this.isLikelyGeminiSuccessBody(geminiBody)) {
      return ctx.response;
    }
  ...
  private isLikelyGeminiSuccessBody(body: any): boolean {
    if (!body || typeof body !== 'object') {
      return false;
    }
  ...
  async processStreamChunk(chunk: any, ctx: StreamChunkContext): Promise<any[] | null> {
    ...
    for (let partIdx = 0; partIdx < parts.length; partIdx++) {
      ...
      if (part.functionCall) {
        ...
        const lastLength = Number(ctx.streamState.get(textKey) ?? 0);
