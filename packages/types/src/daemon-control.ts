...
type DaemonMetadataServing = DaemonMetadataBase & {
  ...
  readonly management_host: '127.0.0.1' | '::1';
...
function managementHost(value: Record<string, unknown>): '127.0.0.1' | '::1' {
  const result = stringField(value, 'management_host');
  if (result !== '127.0.0.1' && result !== '::1') fail('invalid_metadata', 'management_host must be loopback');
  return result;
}
