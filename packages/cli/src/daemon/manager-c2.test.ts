...
async function armedFixture(directory: string, state: 'armed' | 'starting' = 'armed', managementHost: '127.0.0.1' = '127.0.0.1', managementPort = 8089): Promise<DaemonMetadataV1> {
  ...
  const armed: DaemonMetadataV1 = { ...starting, state: 'armed', instance_id: '55555555-5555-4555-8555-555555555555', management_host: managementHost, management_port: managementPort };
  await transitionDaemonMetadataFile(path, {
...
describe('DaemonManager Stage C-2 stop', () => {
  ...
  test('uses the private loopback metadata without changing the exact shutdown path', async () => {
    const directory = makeCanonicalTempDir('bungee-c2-loopback', { daemonSafe: true });
    directories.push(directory);
    const metadata = await armedFixture(directory, 'armed', '127.0.0.1', 18089);
    ...
    await manager.stop();
    expect(requestUrl).toBe('http://127.0.0.1:18089/__bungee/internal/daemon/v1/shutdown');
  });
