...
export async function probeDaemonProcessUser(pid: number, options: ProcessIdentityProbeOptions = {}): Promise<ProcessUserProbe> {
  ...
  try {
    ...
    ({ stdout } = await (options.execFile ?? execFile)(resolveWindowsPowerShell(), ['-NoProfile', '-NonInteractive', '-Command',
      `$ErrorActionPreference='Stop'; $p=@([System.Management.ManagementObjectSearcher]::new('SELECT Handle,ProcessId FROM Win32_Process WHERE ProcessId=${pid}').Get()); if($p.Count -eq 0){exit 3}; if($p.Count -ne 1){exit 4}; $h=$p[0].Properties['Handle'].Value; if($null -eq $h){exit 4}; $hk=[uint32]0; if(-not [uint32]::TryParse([string]$h,[ref]$hk)){exit 4}; if($hk -ne [uint32]${pid}){exit 4}; $options=[System.Management.InvokeMethodOptions]::new(); $out=$p[0].InvokeMethod('GetOwnerSid',$null,$options); if($null -eq $out){exit 4}; $rv=$out.GetPropertyValue('ReturnValue'); if($null -eq $rv -or [uint32]$rv -ne 0){exit 4}; $o=[string]$out.GetPropertyValue('Sid'); if([string]::IsNullOrEmpty($o)){exit 4}; $current=[System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value; [Console]::Write(\"$o|$current\")`,
    ], EXEC_OPTIONS));
