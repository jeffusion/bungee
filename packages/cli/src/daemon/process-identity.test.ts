...
describe('CLI process identity parsing', () => {
  test('requires an exact marker token', () => {
    expect(exactBootMarker(['bun', '/tmp/main.ts', `--bungee-daemon-boot=${BOOT}`], BOOT)).toBe(true);
  ...
  test('only treats explicit target-missing probes as dead', async () => {
    const expected = { executable: 'C:\\Bun\\bun.exe', entrypoint: null };
    ...
    expect(await probeDaemonProcess(42, { executable: '/usr/bin/bun', entrypoint: null }, BOOT, { platform: 'darwin', execFile: psMissing })).toBe('dead');
  ...
  test('does not turn unrelated /proc errors into dead', async () => {
    const expected = { executable: '/usr/bin/bun', entrypoint: null };
  ...
  test('normalizes only the Linux proc deleted suffix and preserves fail-closed identity results', async () => {
    ...
    const deleted = await probeDaemonProcess(42, { executable: '/usr/bin/bun', entrypoint: null }, BOOT, {
      platform: 'linux', readFile: readFile as never, realpath: (async () => '/usr/bin/bun (deleted)') as never,
    });
    expect(deleted).toBe('exact');
    expect(await probeDaemonProcess(42, { executable: '/usr/bin/bun (deleted)', entrypoint: null }, BOOT, {
      platform: 'linux', readFile: readFile as never, realpath: (async () => '/usr/bin/bun (deleted)') as never,
    })).toBe('unknown');
    const noMarker = async (path: string) => path.endsWith('/stat') ? stat : Buffer.from('bun\0other\0');
    expect(await probeDaemonProcess(42, { executable: '/usr/bin/bun', entrypoint: null }, BOOT, {
  ...
  test('Darwin uses fixed-locale raw command topology and lsof executable paths with spaces', async () => {
    const directory = await mkdtemp('/tmp/bungee darwin snapshot ');
