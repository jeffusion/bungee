...
export class DaemonManager {
  ...
  private async requestShutdown(metadata: Extract<DaemonMetadataV1, { state: 'armed' | 'stopping' }>, timeoutMs = this.rpcTimeoutMs): Promise<boolean> {
    if (metadata.management_host !== '127.0.0.1' || isIP(metadata.management_host) === 0 || !Number.isSafeInteger(metadata.management_port)
      || metadata.management_port < 1 || metadata.management_port > 65_535) throw new Error('Daemon master control endpoint is invalid');
    const host = metadata.management_host;
