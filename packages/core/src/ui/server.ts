...
async function bundledUI(request: Request, requestPath: string): Promise<Response> {
  ...
  if (!requestPath.includes('.') && fallback !== null) {
    return responseForAsset(request, fallback, {
      'Content-Type': 'text/html',
      'Content-Length': String(assetLength(fallback)),
    });
  }
...
export function createMasterUIHandler(options: MasterUIHandlerOptions): MasterUIHandler {
  return async (request: Request): Promise<Response | null> => {
    ...
    if (pathname !== '/__ui' && !pathname.startsWith('/__ui/')) return null;

    const requestPath = pathname.slice('/__ui'.length) || '/';
    ...
    if (requestPath === '/plugins') return response(request, 'Not Found', { status: 404 });
    if (requestPath.startsWith('/plugins/')) {
      const rawPluginPath = requestPath.slice('/plugins/'.length);
    ...
    const decodedPath = decodePath(requestPath);
