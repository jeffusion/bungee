...
export class WorkerSupervisionHttpServer {
  ...
  private async command(request: Request): Promise<Response> {
    ...
    const result = await this.commands.execute(message as CommandEnvelope, `worker:${message.request_id}`, async () => {
      if (this.isFrozen()) throw new SupervisionProtocolError('worker_frozen', 'worker supervision is frozen');
      const commandResult = await this.executeCommand(message.path, root.body);
      this.evidence = commandResult.evidence;
      this.isFrozen();
      await this.refreshDescriptorBestEffort();
      return this.signedStatus(authority, message.request_id);
    });
  ...
  private async executeCommand(path: string, value: unknown): Promise<{ readonly runtime?: ConfigWorkerRuntimeResult; readonly evidence: WorkerStatusEvidence }> {
    ...
    if (path === '/shutdown') {
      ...
      this.frozen = true;
      this.publishAuthorityIfChanged();
      // Let Bun flush the signed command evidence before closing its control listener.
      setTimeout(() => { void Promise.resolve(this.onShutdown?.()).catch(() => undefined); }, 25);
