...
function parseStatusBody(value: unknown): WorkerStatusPayload {
  ...
  return {
    ...
    control_port: integer(input.control_port, 'control_port', 1, 65_535), phase, frozen: input.frozen,
...
function abortable<Result>(operation: PromiseLike<Result>, signal: AbortSignal): Promise<Result> {
  return new Promise<Result>((resolve, reject) => {
    ...
    const abort = () => rejectOnce(new WorkerControllerClientError('timeout', 'worker runtime snapshot timed out'));
...
export class WorkerControllerClient {
  ...
  private enqueueRuntime<T>(operation: () => Promise<T>, signal: AbortSignal, deadline: number): Promise<T> {
    const queued = this.queue.then(() => {
      if (this.stopped || signal.aborted || Date.now() >= deadline) {
  ...
  private async fetchJson(path: string, payload: unknown, signal?: AbortSignal): Promise<unknown> {
      ...
      try {
        ...
        try {
          const responseTimeout = new Promise<never>((_, reject) => {
            responseTimer = setTimeout(() => reject(new WorkerControllerClientError('timeout', 'worker supervision response timed out')), this.timeoutMs);
          });
          while (true) {
            const result = await Promise.race([reader.read(), responseTimeout]);
        ...
        if (attempt === 1 || (lastError instanceof WorkerControllerClientError
          && (lastError.code === 'protocol' || lastError.code === 'response_too_large'))) {
          if (attempt === 1 && transportFailed) this.setState('unavailable');
          throw lastError;
        }
      } finally {
        clearTimeout(timer);
    ...
    if (transportFailed) this.setState('unavailable');
  ...
  private acceptStatus(response: unknown, requestId: string): WorkerStatusPayload {
  ...
  private async fetchRuntimeJson(payload: unknown, signal: AbortSignal): Promise<unknown> {
    ...
    try {
      ...
      try {
        ...
        if (this.stopped || signal.aborted) throw new WorkerControllerClientError('timeout', 'worker runtime snapshot timed out');
        return parsed;
      } finally {
        cancelBestEffort(() => reader.cancel());
      }
  ...
  runtimeSnapshot(signal?: AbortSignal, deadline = Date.now() + RUNTIME_SNAPSHOT_TIMEOUT_MS): Promise<WorkerRuntimeSnapshot> {
    if (this.stopped || signal?.aborted) return Promise.reject(new WorkerControllerClientError('timeout', 'worker runtime snapshot timed out'));
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort('deadline'), Math.max(0, deadline - Date.now()));
    ...
    return this.enqueueRuntime(async () => {
      try {
        if (this.stopped || controller.signal.aborted || Date.now() >= deadline) throw new WorkerControllerClientError('timeout', 'worker runtime snapshot timed out');
  ...
  private async attachOnce(): Promise<WorkerStatusPayload> {
    const challengeNonce = await this.challenge();
    ...
    try {
      attached = this.acceptStatus(await this.fetchJson('/__supervision/attach', attach), attachId);
    } catch (error) {
      if (!(error instanceof WorkerControllerClientError)
        || (error.code !== 'timeout' && error.code !== 'http' && error.code !== 'network')) throw error;
      try { attached = await this.statusOnce(); }
      catch { throw error; }
    }
    ...
    const leased = this.acceptStatus(await this.fetchJson('/__supervision/lease', lease), leaseId);
  ...
  private scheduleRenewal(): void {
    ...
    this.renewTimer = setTimeout(() => {
      void this.enqueue(async () => {
        try { await this.renewOnce(); }
        catch { this.scheduleRecovery(); }
      }).catch(() => undefined);
    }, Math.max(1, this.leaseDurationMs - this.renewBeforeMs));
  }

  private async renewOnce(): Promise<WorkerStatusPayload> {
    const requestId = randomUUID();
    const sequence = this.nextSequence();
    const lease = signSupervisionMessage({ protocol: 'bungee-supervision-v1', kind: 'lease', direction: 'controller-to-process',
      ...this.options.credential.identity, ...this.options.authority, sequence, request_id: requestId,
      lease_expires_at: Date.now() + this.leaseDurationMs }, this.options.credential);
    const result = this.acceptStatus(await this.fetchJson('/__supervision/lease', lease), requestId);
    this.setState('attached');
    this.scheduleRenewal();
    return result;
  }
  ...
  private scheduleRecovery(): void {
    ...
    this.renewTimer = setTimeout(() => {
      void this.enqueue(() => this.attachOnce()).catch(() => this.scheduleRecovery());
    }, delay);
  }

  lease(): Promise<WorkerStatusPayload> { return this.enqueue(() => this.renewOnce()); }

  status(): Promise<WorkerStatusPayload> {
    return this.enqueue(() => this.statusOnce());
  }

  private async statusOnce(): Promise<WorkerStatusPayload> {
      const requestId = randomUUID();
      const sequence = this.nextSequence();
      ...
      return this.acceptStatus(await this.fetchJson('/__supervision/status', message), requestId);
  }
  ...
  command(message: ConfigMasterMessage): Promise<WorkerStatusPayload> {
    return this.enqueue(async () => {
      ...
      if (path === null) throw new WorkerControllerClientError('protocol', 'unsupported worker command');
      return this.commandOnce(path, message);
    });
  }

  private async commandOnce(path: '/start' | '/drain' | '/shutdown', body: unknown): Promise<WorkerStatusPayload> {
    const requestId = randomUUID();
    const sequence = this.nextSequence();
    ...
    return this.acceptStatus(await this.fetchJson('/__supervision/command', { message, body }), requestId);
  }

  start(message: ConfigMasterMessage): Promise<WorkerStatusPayload> { return this.command(message); }

  drain(message: ConfigMasterMessage): Promise<WorkerStatusPayload> { return this.command(message); }

  shutdown(): Promise<WorkerStatusPayload> {
    return this.enqueue(() => this.commandOnce('/shutdown', {}));
  }

  disconnect(expireLease = true): void {
    this.stopped = true;
    for (const controller of this.runtimeControllers) controller.abort('disconnected');
    if (expireLease && this.state === 'attached') {
      const requestId = randomUUID();
      const sequence = this.nextSequence();
      const lease = signSupervisionMessage({ protocol: 'bungee-supervision-v1', kind: 'lease', direction: 'controller-to-process',
        ...this.options.credential.identity, ...this.options.authority, sequence, request_id: requestId,
