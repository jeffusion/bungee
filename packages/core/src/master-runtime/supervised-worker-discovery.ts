...
export type WorkerDiscoveryIssue = {
  ...
  readonly kind: 'malformed' | 'tampered' | 'stale' | 'unreachable' | 'duplicate' | 'master_control_mismatch';
...
function sameStatusEvidence(descriptor: WorkerDescriptor, status: WorkerStatusPayload): boolean {
  return descriptor.master_generation === status.master_generation && descriptor.worker_instance_id === status.worker_instance_id
    ...
    && descriptor.master_control_port === status.master_control_port
...
export async function discoverSupervisedWorkers(options: WorkerDiscoveryOptions): Promise<{
  ...
  const masterControlPort = options.masterControlPort;
  ...
  for (const candidate of selected) {
    ...
    try {
      ...
      const masterControlMismatch = status.master_control_port !== masterControlPort
        || liveDescriptor.master_control_port !== masterControlPort;
      if (options.mode === 'exact-admission' && masterControlMismatch) {
        issues.push(issue(file, 'master_control_mismatch', new Error('worker master control port is missing or does not match the current master')));
        continue;
      }
      if (status.worker_instance_id !== identity.worker_instance_id || status.worker_slot !== identity.worker_slot
         ...
         || status.control_port !== candidate.descriptor.control_port
         || !sameStatusEvidence(liveDescriptor, status)) throw new SupervisionProtocolError('malformed_message', 'online status identity does not match descriptor or master control port');
