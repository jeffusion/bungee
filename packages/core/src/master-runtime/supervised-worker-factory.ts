...
export class SupervisedConfigWorkerFactory implements ConfigPublicationWorkerFactory {
  ...
  constructor(private readonly options: SupervisedConfigWorkerFactoryOptions) {
    ...
    const host = options.managementHost;
    const port = options.managementPort;
    if ((host !== '127.0.0.1' && host !== '::1') || !Number.isSafeInteger(port) || port < 1 || port > 65_535) {
      throw new Error('supervised management address is invalid');
    }
  ...
  spawn(identity: ConfigProcessIdentity): SupervisedConfigWorkerProcessAdapter {
    ...
    const child = this.spawnWorker(this.options.launch.executable, [...this.options.launch.args,
      ...
      env: {
        ...
        [CONFIG_WORKER_ENV_NAMES.managementHost]: this.options.managementHost,
        [CONFIG_WORKER_ENV_NAMES.managementPort]: String(this.options.managementPort),
