...
export function parseSupervisedWorkerEnvironment(
  ...
  const masterControlPort = integer(required(env, CONFIG_WORKER_ENV_NAMES.masterControlPort), CONFIG_WORKER_ENV_NAMES.masterControlPort, 1);
  if (masterControlPort > 65_535) throw new Error(`${CONFIG_WORKER_ENV_NAMES.masterControlPort} must be a safe integer <= 65535`);
