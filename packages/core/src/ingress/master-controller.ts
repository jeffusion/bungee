...
export type MasterIngressMutationReadinessReason =
  | 'not_attached'
  | 'authority_mismatch'
  | 'lease_margin'
  | 'active_missing'
  | 'prepared_pending'
  | 'retired_pending'
  | 'retired_release_pending'
  | 'active_operation'
  | 'active_revision_mismatch'
  | 'active_content_mismatch'
  | 'plugin_catalog_mismatch';
export type MasterIngressMutationReadiness =
  | { readonly ready: true }
  | { readonly ready: false; readonly reason: MasterIngressMutationReadinessReason };
export type MasterIngressMutationReadinessEvidence = {
...
type RecoveryControlResult = {
  readonly recovered: boolean;
  readonly event: 'new_boot' | 'same_boot' | null;
};
...
export class MasterIngressController implements WorkerAdmissionController {
  ...
  mutationReadiness(evidence?: MasterIngressMutationReadinessEvidence): MasterIngressMutationReadiness {
    ...
    if (this.client === null || this.state !== 'attached' || status === null || status.state !== 'attached'
      || this.trustedStatusAt < 0) return { ready: false, reason: 'not_attached' };
    if (this.trustedStatusAuthority?.controller_id !== this.authority.controller_id
      || this.trustedStatusAuthority?.controller_epoch !== this.authority.controller_epoch) {
      return { ready: false, reason: 'authority_mismatch' };
    }
    if (this.monotonicNow() >= this.leaseDeadline - freshnessMargin) return { ready: false, reason: 'lease_margin' };
    const active = status.registry.active;
    if (active === null) return { ready: false, reason: 'active_missing' };
    if (status.registry.prepared !== null) return { ready: false, reason: 'prepared_pending' };
    if (status.registry.retired.length > 0) return { ready: false, reason: 'retired_pending' };
    if (this.pendingRetiredRelease !== null) return { ready: false, reason: 'retired_release_pending' };
    if (evidence === undefined) return { ready: true };
    if (evidence.hasActiveOperation) return { ready: false, reason: 'active_operation' };
    if (active.revision !== evidence.revision) return { ready: false, reason: 'active_revision_mismatch' };
    if (active.content_hash !== evidence.content_hash) return { ready: false, reason: 'active_content_mismatch' };
    if (active.plugin_catalog_hash !== evidence.plugin_catalog_hash) return { ready: false, reason: 'plugin_catalog_mismatch' };
    return { ready: true };
  }

  isMutationReady(evidence?: MasterIngressMutationReadinessEvidence): boolean {
    return this.mutationReadiness(evidence).ready;
  }
  ...
  prepare(workers: readonly ServingConfigWorker[], signal?: AbortSignal): Promise<PreparedWorkerAdmission> {
    ...
    const promise = (async (): Promise<PreparedWorkerAdmission> => {
      ...
      try {
        ...
        const control = await this.recoverControl();
        if (control.recovered) {
          if (control.event === 'new_boot') void this.invokeRecovered().catch(() => undefined);
          else await this.invokeRecovered();
        }
        this.assertAdmissionHandleScope(scope);
          ...
          const run = async (): Promise<void> => {
            try {
              ...
              try {
                ...
                const recovery = await this.recoverControl();
                if (recovery.recovered) {
                  if (recovery.event === 'new_boot') void this.invokeRecovered().catch(() => undefined);
                  else await this.invokeRecovered();
                }
                this.assertAdmissionHandleScope(scope);
  ...
  async recover(): Promise<void> {
    const recovery = await this.recoverControl();
    if (recovery.recovered) await this.invokeRecovered();
  }

  private async recoverControl(): Promise<RecoveryControlResult> {
    if (this.client === null) throw new MasterIngressControllerError('not_attached', 'ingress controller is not connected');
    this.cancelTimer();
    const boot = this.bootGeneration;
    ...
    if (!recovered) return { recovered: false, event: null };
    return { recovered: true, event: this.bootGeneration === boot ? 'same_boot' : 'new_boot' };
  }
  ...
  private async recoverNow(signal?: AbortSignal): Promise<boolean> {
    ...
    try {
      ...
      let newBoot = false;
      if (this.pendingPrepareRecovery === null && this.uncertainAdmission === null) {
        try {
          await this.statusNow(undefined, signal);
          this.assertRecoveryEligible(lifecycle);
        } catch (cause) {
