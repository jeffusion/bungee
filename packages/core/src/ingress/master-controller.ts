...
export class MasterIngressController implements WorkerAdmissionController {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      await this.enqueue(() => this.client!.lease(
        this.authority,
  ...
  prepare(workers: readonly ServingConfigWorker[], signal?: AbortSignal): Promise<PreparedWorkerAdmission> {
    ...
    const promise = (async (): Promise<PreparedWorkerAdmission> => {
      ...
      try {
        ...
        await this.enqueue(() => {
          throwIfAborted(signal);
          return this.command('/prepare', set, scope);
        });
        this.assertAdmissionHandleScope(scope);
        throwIfAborted(signal);
        previousActive = (await this.status(scope)).registry.active;
        this.assertAdmissionHandleScope(scope);
      }
      catch (cause) {
        this.assertAdmissionHandleScope(scope);
        const status = await this.statusAfterFailure(cause, scope);
        this.assertAdmissionHandleScope(scope);
        previousActive = status.registry.active;
        const identity = admissionSetIdentity(set);
        const prepared = status.registry.prepared !== null && admissionSetIdentity(status.registry.prepared) === identity;
        const active = status.registry.active !== null && admissionSetIdentity(status.registry.active) === identity;
      ...
      return {
        commit: async () => {
          const run = async (): Promise<void> => {
            try {
              throwIfAborted(signal);
              this.assertAdmissionHandleScope(scope);
              const outcome = await this.enqueue(() => this.commitAdmission(set, scope));
              throwIfAborted(signal);
              this.assertAdmissionHandleScope(scope);
              await this.resolveAdmission(set, outcome, scope);
              this.assertAdmissionHandleScope(scope);
        ...
        abort: async () => {
          try { this.assertAdmissionHandleScope(scope); await this.abortAdmission(set, scope); }
          finally { if (this.pendingAdmission === pending) this.pendingAdmission = null; }
        },
        releaseRetiredAfterExitProof: async () => {
          try {
            this.assertAdmissionHandleScope(scope);
            await this.enqueue(() => this.releaseRetiredAfterExitProof(previousActive, scope));
          } finally {
  ...
  async status(scope?: AdmissionHandleScope): Promise<IngressStatusPayload> {
    ...
    try {
      const status = await client.status(this.authority, this.nextSequence());
      if (scope !== undefined) this.assertAdmissionHandleScope(scope);
  ...
  async fenceAndStatus(token?: IngressBootRecoveryToken): Promise<IngressStatusPayload['registry']> {
    ...
    try {
      return await this.enqueue(async () => {
        this.assertAdmissionHandleScope(scope);
        if (token !== undefined) this.assertCurrentRecoveryToken(token);
        const status = await this.client!.fence(this.authority, this.nextSequence());
        this.assertAdmissionHandleScope(scope);
        if (token !== undefined) this.assertCurrentRecoveryToken(token);
        this.trustStatus(status);
  ...
  async recover(): Promise<void> {
    if (this.client === null) throw new MasterIngressControllerError('not_attached', 'ingress controller is not connected');
    this.cancelTimer();
    const recovered = await this.enqueue(() => this.recoverNow());
  ...
  private async spawnAndAttach(previous?: ProcessIdentity): Promise<Extract<MasterIngressRecoveryEvent, { readonly kind: 'new_boot' }> | null> {
    const lifecycle = this.lifecycleGeneration;
    ...
    const identity = await this.waitForIdentityAfterSpawn(childEnvironment);
  ...
  private async waitForIdentityAfterSpawn(environment: NodeJS.ProcessEnv): Promise<import('../supervision').ProcessIdentity> {
    ...
    while (Date.now() < deadline) {
      try {
        const identity = await discoverIngressIdentity(this.baseUrl(), this.fetch, DISCOVERY_TIMEOUT_MS);
        this.assertSpawnEligible(lifecycle);
        return identity;
      }
      catch (error) {
  ...
  private scheduleLeaseRenewal(): void {
    ...
    this.leaseTimer = setTimeout(() => {
      ...
      void this.enqueue(async () => {
        ...
        try {
          const leaseDeadline = this.monotonicNow() + this.leaseDurationMs;
          await this.client.lease(this.authority, this.now() + this.leaseDurationMs, this.nextSequence());
          if (!this.isRecoveryEligible(lifecycle)) return;
          this.leaseDeadline = leaseDeadline;
          await this.status();
  ...
  private scheduleRecovery(): void {
    ...
    this.leaseTimer = setTimeout(() => {
      ...
      if (!this.isRecoveryEligible(lifecycle)) return;
      void this.enqueue(() => this.recoverNow()).then(
        async (recovered) => {
          if (!this.isRecoveryEligible(lifecycle)) return;
          await this.resolvePendingAdmission();
          if (recovered) await this.invokeRecovered();
  ...
  private async recoverNow(): Promise<boolean> {
    if (this.client === null) throw new MasterIngressControllerError('not_attached', 'ingress controller is not connected');
    const lifecycle = this.lifecycleGeneration;
    this.assertRecoveryEligible(lifecycle);
    try {
      ...
      try {
        ...
        const event = await this.recoverNewBoot(cause);
      ...
      if (!newBoot && this.uncertainAdmission !== null) {
        await this.resolveUncertain(this.trustedStatus!);
      }
      const pendingRelease = this.pendingRetiredRelease;
      if (!newBoot && pendingRelease !== null) await this.releaseRetiredAfterExitProof(pendingRelease.set);
  ...
  private async recoverNewBoot(_cause: unknown): Promise<MasterIngressRecoveryEvent | null> {
    const lifecycle = this.lifecycleGeneration;
    if (!this.isRecoveryEligible(lifecycle)) return null;
    ...
    try {
      candidate = await discoverIngressIdentity(this.baseUrl(), this.fetch, DISCOVERY_TIMEOUT_MS);
    } catch (error) {
      if (!(error instanceof IngressDiscoveryError) || error.code !== 'unavailable') throw error;
      if (!this.isRecoveryEligible(lifecycle)) return null;
      return this.spawnAndAttach(previous);
    }
    if (!this.isRecoveryEligible(lifecycle)) return null;
  ...
  private async resolvePendingAdmission(): Promise<void> {
    const uncertain = this.uncertainAdmission;
    if (uncertain?.pendingResolution !== null && uncertain?.pendingResolution !== undefined) {
      await this.resolveAdmission(uncertain.target, uncertain.pendingResolution);
    }
  }
  ...
  private requireMutation(): void {
    ...
    if (this.pendingRetiredRelease !== null
  ...
  private async command(path: string, body: unknown, scope?: AdmissionHandleScope): Promise<unknown> {
    if (scope !== undefined) this.assertAdmissionHandleScope(scope);
    if (this.client === null) throw new MasterIngressControllerError('not_attached', 'ingress controller is not connected');
    ...
    try {
      const client = this.client;
      const result = await client.command(this.authority, this.nextSequence(), path, body);
      if (scope !== undefined) this.assertAdmissionHandleScope(scope);
      return result;
    } catch (error) {
      this.clearFreshness();
  ...
  private async commitAdmission(set: AdmissionSet, scope?: AdmissionHandleScope): Promise<'committed' | 'not_committed'> {
    if (scope !== undefined) this.assertAdmissionHandleScope(scope);
    ...
    try {
      await this.command('/commit', set, scope);
      status = await this.status(scope);
    } catch (cause) {
      status = await this.statusAfterFailure(cause, scope);
    }
    if (scope !== undefined) this.assertAdmissionHandleScope(scope);
    const outcome = await this.resolveUncertain(status, scope);
    if (outcome !== null) return outcome;
    this.admissionOutcomeUnknown('ingress commit was not confirmed active', status);
  }

  private async resolveAdmission(target: AdmissionSet, outcome: 'committed' | 'not_committed', scope?: AdmissionHandleScope): Promise<void> {
    if (scope !== undefined) this.assertAdmissionHandleScope(scope);
    const uncertain = this.uncertainAdmission;
    if (uncertain === null || admissionSetIdentity(uncertain.target) !== admissionSetIdentity(target)) {
  ...
  private async resolveUncertain(status: IngressStatusPayload, scope?: AdmissionHandleScope): Promise<'committed' | 'not_committed' | null> {
    if (scope !== undefined) this.assertAdmissionHandleScope(scope);
    const uncertain = this.uncertainAdmission;
    if (uncertain === null || this.resolvingUncertain) return null;
    const targetIdentity = admissionSetIdentity(uncertain.target);
    ...
    try {
      ...
      if (status.registry.prepared !== null && admissionSetIdentity(status.registry.prepared) === targetIdentity) {
        try { await this.command('/commit', uncertain.target, scope); }
        catch (cause) { await this.statusAfterFailure(cause, scope); }
        const confirmed = await this.status(scope);
      ...
      try {
        const client = this.client;
        await client.fence(this.authority, this.nextSequence());
        if (scope !== undefined) this.assertAdmissionHandleScope(scope);
        const fenced = await this.status(scope);
        ...
        if (fenced.registry.prepared !== null && admissionSetIdentity(fenced.registry.prepared) === targetIdentity) {
          await this.command('/commit', uncertain.target, scope);
          const committed = await this.status(scope);
  ...
  private async releaseRetiredAfterExitProof(previousActive: AdmissionSet | null, scope?: AdmissionHandleScope): Promise<void> {
    if (scope !== undefined) this.assertAdmissionHandleScope(scope);
    ...
    try {
      let status = await this.releaseRetiredAttempt(set, scope);
      if (status.registry.retired.some((candidate) => admissionSetIdentity(candidate) === identity)) {
        status = await this.releaseRetiredAttempt(set, scope);
        if (status.registry.retired.some((candidate) => admissionSetIdentity(candidate) === identity)) {
  ...
  private async releaseRetiredAttempt(set: AdmissionSet, scope?: AdmissionHandleScope): Promise<IngressStatusPayload> {
    try {
      await this.command('/release-retired', set, scope);
  ...
  private async abortAdmission(set: AdmissionSet, scope?: AdmissionHandleScope): Promise<void> {
    try {
      await this.enqueue(() => this.command('/abort', set, scope));
    } catch (cause) {
      const status = await this.statusAfterFailure(cause, scope);
  ...
  private async statusAfterFailure(cause: unknown, scope?: AdmissionHandleScope): Promise<IngressStatusPayload> {
    if (scope !== undefined) this.assertAdmissionHandleScope(scope);
    try {
      if (this.client === null) throw new MasterIngressControllerError('not_attached', 'ingress controller is not connected');
      const client = this.client;
