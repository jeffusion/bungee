...
async function hostileCancel(mode: 'resolve' | 'reject'): Promise<{ readonly cancelCount: number; readonly bodyCancelCount: number }> {
  ...
  const bodyCanceled = new Promise<void>((resolve) => { resolveBodyCanceled = resolve; });
  const body = new ReadableStream<Uint8Array>({ cancel() { bodyCancelCount += 1; resolveBodyCanceled(); } });
  ...
  let cancelDeadlineAt = 0;
  let cancelAbortedAt = 0;
  let resolveCancelAborted!: () => void;
  const cancelAborted = new Promise<void>((resolve) => { resolveCancelAborted = resolve; });
  const client = createPluginControlHttpClient({
    ...
    fetchImpl: (async (_input, init) => {
      ...
      if (message.kind === 'cancel') {
        ...
        cancelDeadlineAt = message.deadline_at;
        init?.signal?.addEventListener('abort', () => { cancelAbortedAt = Date.now(); resolveCancelAborted(); }, { once: true });
...
test('higher epoch attach cancels old authority once before the new lease call', async () => {
  ...
  try {
    ...
    expect(cancelCount).toBe(1);
    expect(hostAborted).toBe(true);
    const newAuthorityCall = bound.call('ok', {}, new AbortController().signal).then((value) => value, (error) => error);
    expect(await newAuthorityCall).toEqual({ sequence: 3 });
  } finally {
    releaseCancel();
    provider.dispose();
    await worker.stop();
...
test('dispose aborts a pending cancel transport and clears its tracking', async () => {
  const bootNonce = '80000000-0000-4000-8000-000000000006';
  const supervision = deriveWorkerSupervisionCredential(
  ...
  const outcome = client.call({ revision: 1, endpoint_id: 'endpoint-1', attempt_id: randomUUID(), method: 'block', payload: {} }, callController.signal)
    .then(() => undefined, (error) => error);
  try {
    await callStarted;
    callController.abort();
    await cancelStarted;
    expect((client as any).cancelTransports.size).toBe(1);
    client.dispose();
