...
describe('asynchronous configuration control', () => {
  ...
  test('serializes same-revision PUTs before operation creation and preflight', async () => {
    const operationA = 'serial-put-a';
    const operationB = 'serial-put-b';
    const controlA = 'serial-control-a';
    const controlB = 'serial-control-b';
    const preflightCalls: string[] = [];
    let signalPreflightStarted!: () => void;
    const preflightStarted = new Promise<void>((resolve) => { signalPreflightStarted = resolve; });
    let releasePreflight!: () => void;
    const { api, repository, publishedOperationIds } = fixture({
      controlNames: new Set([controlA, controlB]),
      async activate(name) {
    ...
    const revision = repository.getSnapshot().revision;
    const aggregateA = { ...disabledAggregate(), plugin_activations: [{ plugin_name: controlA }] };
    const aggregateB = { ...disabledAggregate(), plugin_activations: [{ plugin_name: controlB }] };

    const first = put(api, revision, aggregateA, operationA, {});
    ...
    expect(await snapshotBeforeCommit?.json()).toMatchObject({ revision });
    expect(preflightCalls).toEqual([controlA]);
    expect(publishedOperationIds).toEqual([]);

    let secondCompleted = false;
    const second = put(api, revision, aggregateB, operationB, {});
    void second.then(() => { secondCompleted = true; });
    await Promise.resolve();
    expect(secondCompleted).toBe(false);
    expect(preflightCalls).toEqual([controlA]);
    expect(publishedOperationIds).toEqual([]);
    expect(repository.getSnapshot().revision).toBe(revision);

    releasePreflight();
    const firstResponse = await first;
    expect(firstResponse?.status).toBe(202);
    expect(repository.getSnapshot().revision).toBe(revision + 1);

    const secondResponse = await second;
    expect(secondResponse?.status).toBe(409);
    expect(await secondResponse?.json()).toMatchObject({
      error: 'stale_revision', expected_revision: revision,
      active: { revision: revision + 1, config: aggregateA },
    });
    ...
    expect(await operationAfterCommit?.json()).toMatchObject({
      operation: { mutation_id: operationA, committed_revision: revision + 1 },
    });
    expect(repository.getOperationState(operationB)).toBeNull();
    expect(repository.getSnapshot().revision).toBe(revision + 1);
    expect(publishedOperationIds).toEqual([operationA]);
    expect(preflightCalls).toEqual([controlA]);
  });
  ...
  test('preserves a committed PUT when acceptCommitted cannot read its operation', async () => {
    ...
    let commitCalls = 0;
    let preCommitSnapshotReads = 0;
    let committed = false;
    let failAcceptCommittedRead = true;
    repository.getSnapshot = () => {
      if (!committed) preCommitSnapshotReads += 1;
      return originalGetSnapshot();
    };
    repository.commit = (command) => {
      commitCalls += 1;
      const result = originalCommit(command);
      if (result.kind === 'committed') committed = true;
      return result;
    };
    repository.getOperationState = (id) => {
      if (committed && failAcceptCommittedRead && id === mutationId) {
        failAcceptCommittedRead = false;
        throw new ConfigRepositoryError('repository_failure', 'operation read failed after commit');
      }
      return originalGetOperationState(id);
    };

    const response = await put(api, 1, next, mutationId, {});
    ...
    expect(operation).toMatchObject({
      mutation_id: mutationId, expected_revision: 1, committed_revision: 2,
      kind: 'config', state: 'committed', target_worker_count: 1,
      request_hash: hashConfigurationRequest({ kind: 'config', expected_revision: 1,
        aggregate: next, target_worker_slots: [0] }),
    });
    if (operation === null) throw new Error('committed operation missing after recovered read');
    expect(repository.getOperationState(mutationId)?.operation).toEqual(operation);
    expect(repository.getActivePublication()).toMatchObject({
      operation: { mutation_id: mutationId, committed_revision: 2 },
      snapshot: { revision: 2, aggregate: next, content_hash: hashConfigurationContent(next) },
    });
    expect(publishedOperationIds).toEqual([]);
    expect(commitCalls).toBe(1);
  });
  ...
  test('rechecks the latest active auth after a slow import body finishes', async () => {
    ...
    const slow = api.handle(new Request('http://control.test/api/config/import', {
      ...
      body: new ReadableStream<Uint8Array>({ start(controller) {
        finishBody = () => {
          controller.enqueue(encoder.encode(JSON.stringify({
            expected_revision: 2, mutation_id: 'slow-old-import', envelope,
          })));
          controller.close();
        };
      } }),
