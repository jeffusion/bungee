...
describe('plugin control host integration', () => {
  ...
  test('cancels pending activation before load resolves and preserves the secret marker', async () => {
    ...
    const host = createPluginControlHost({
      ...
      loadControl: async () => { loading(); await loadRelease; return { createControl: () => ({ api: [], rpc: [], start() { starts++; }, dispose() { disposes++; } }) }; },
    });
    ...
    const deactivationOutcome = deactivation.then(
      () => ({ ok: true as const }),
      (error) => ({ ok: false as const, error }),
    );
    let shutdownSettled = false;
    const shutdownOutcome = host.dispose().then(
      () => { shutdownSettled = true; return { ok: true as const }; },
      (error) => { shutdownSettled = true; return { ok: false as const, error }; },
    );
    ...
    try {
      ...
      await Promise.resolve();
      expect(db.query<{ count: number }, []>('SELECT count(*) AS count FROM secret_store_objects').get()?.count).toBe(1);
      expect(shutdownSettled).toBe(false);
      release();
      const activationResult = await activationOutcome;
      expect(activationResult.ok).toBe(false);
      if (activationResult.ok) throw new Error('pending activation unexpectedly succeeded');
      expect(activationResult.error).toMatchObject({ code: 'disposed' });
      const deactivationResult = await deactivationOutcome;
      if (!deactivationResult.ok) throw deactivationResult.error;
      const shutdownResult = await shutdownOutcome;
      ...
      const deactivationResult = await deactivationOutcome;
      const shutdownResult = await shutdownOutcome;
      if (!deactivationResult.ok) cleanupErrors.push(deactivationResult.error);
      if (!shutdownResult.ok) cleanupErrors.push(shutdownResult.error);
