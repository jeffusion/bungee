...
async function environment(directory: string, controlPort: number | string = 0): Promise<NodeJS.ProcessEnv> {
  ...
  return {
    ...
    BUNGEE_WORKER_CONTROL_PORT: String(controlPort), BUNGEE_MASTER_CONTROL_PORT: String(await unusedPort()),
    BUNGEE_WORKER_DESCRIPTOR_PATH: join(directory, 'worker.json'),
