...
async function fixture() {
  ...
  await createLaunchingDaemonMetadataFile(path, metadata, { runtimeDirectory: dir });
...
describe('daemon bootstrap takeover', () => {
  ...
  test('rejects partial, marker, secret, and identity mismatches before any composition dependency', async () => {
    ...
    await expect(takeOverDaemonBootstrap({ env: partial, marker: null, runtimeDirectory: dirname(path) })).rejects.toThrow();
    expect(partial[DAEMON_BOOTSTRAP_ENV_NAMES.metadataPath]).toBeUndefined();

    const mismatch = env(path);
    await expect(takeOverDaemonBootstrap({ env: mismatch, marker: '00000000-0000-0000-0000-000000000000', runtimeDirectory: dirname(path) })).rejects.toThrow();
    const badSecret = env(path, 'AQECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8');
    await expect(takeOverDaemonBootstrap({ env: badSecret, marker: BOOT, runtimeDirectory: dirname(path) })).rejects.toThrow();
    const badIdentity = env(path);
    await expect(takeOverDaemonBootstrap({ env: badIdentity, marker: BOOT, runtimeDirectory: dirname(path), identity: { executable: '/other', entrypoint: null } })).rejects.toThrow();
  });
  ...
  test('leaves starting metadata when transition confirmation fails', async () => {
    ...
    await expect(takeOverDaemonBootstrap({
      env: environment, marker: BOOT, runtimeDirectory: dirname(path),
      identity: { executable: process.execPath, entrypoint: null },
      readMetadata: async (metadataPath, file) => {
        reads += 1;
    ...
    const metadata = await readDaemonMetadataFile(path, { runtimeDirectory: dirname(path) });
