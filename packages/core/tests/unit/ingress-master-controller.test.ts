...
function waitWithTimeout<T>(promise: Promise<T>, message: string, timeoutMs = 500): Promise<T> {
...
test('reports every mutation readiness reason in contract order', async () => {
  const active = {
    master_generation: GENERATION, admission_sequence: 1, revision: 1,
    content_hash: HASH, plugin_catalog_hash: CATALOG,
    workers: [{ master_generation: GENERATION, worker_instance_id: worker().process.identity.worker_instance_id,
  ...
  const evidence = { revision: 1, content_hash: HASH, plugin_catalog_hash: CATALOG, hasActiveOperation: false };
  const readyController = (registry: IngressStatusPayload['registry'] = { active, prepared: null, retired: [] }) => {
    let monotonicNow = 1_000;
    ...
    internal.trustedStatus = status(registry);
    internal.trustedStatusAt = 1_000;
    internal.trustedStatusAuthority = { controller_id: '30000000-0000-4000-8000-000000000001', controller_epoch: 1 };
    internal.leaseDeadline = 1_100;
    return { controller, internal, setMonotonic: (value: number) => { monotonicNow = value; } };
  };

  expect(new MasterIngressController(options()).mutationReadiness()).toEqual({ ready: false, reason: 'not_attached' });
  {
    const { controller, internal } = readyController();
    internal.trustedStatusAuthority = { controller_id: '30000000-0000-4000-8000-000000000099', controller_epoch: 1 };
    expect(controller.mutationReadiness()).toEqual({ ready: false, reason: 'authority_mismatch' });
    await controller.disconnect();
  }
  {
    const { controller, setMonotonic } = readyController();
    setMonotonic(1_070);
    expect(controller.mutationReadiness()).toEqual({ ready: false, reason: 'lease_margin' });
    await controller.disconnect();
  }
  for (const [reason, registry] of [
    ['active_missing', { active: null, prepared: null, retired: [] }],
    ['prepared_pending', { active, prepared: worker() as never, retired: [] }],
    ['retired_pending', { active, prepared: null, retired: [worker() as never] }],
  ] as const) {
    const { controller } = readyController(registry);
    expect(controller.mutationReadiness()).toEqual({ ready: false, reason });
    await controller.disconnect();
  }
  {
    const { controller, internal } = readyController();
    internal.pendingRetiredRelease = {};
    expect(controller.mutationReadiness()).toEqual({ ready: false, reason: 'retired_release_pending' });
    await controller.disconnect();
  }
  for (const [reason, candidate] of [
    ['active_operation', { ...evidence, hasActiveOperation: true }],
    ['active_revision_mismatch', { ...evidence, revision: 2 }],
    ['active_content_mismatch', { ...evidence, content_hash: CATALOG }],
    ['plugin_catalog_mismatch', { ...evidence, plugin_catalog_hash: HASH }],
  ] as const) {
    const { controller } = readyController();
    expect(controller.mutationReadiness(candidate)).toEqual({ ready: false, reason });
    await controller.disconnect();
  }
  {
    const { controller } = readyController();
    expect(controller.mutationReadiness(evidence)).toEqual({ ready: true });
    expect(controller.isMutationReady(evidence)).toBeTrue();
    await controller.disconnect();
  }
});
...
test('a new ingress boot drops old pending, uncertain, and retired state without resolving it', () => {
  const controller = new MasterIngressController(options());
  const target = {
    master_generation: GENERATION, admission_sequence: 1, revision: 1,
    content_hash: HASH, plugin_catalog_hash: CATALOG,
  ...
  const internal = controller as unknown as {
    uncertainAdmission: unknown;
    pendingPrepareRecovery: unknown;
    pendingRetiredRelease: unknown;
    resetForNewBoot(): void;
  };
  internal.uncertainAdmission = { target, previousActive: null, pendingResolution: null, resolutionPromise: null };
  internal.pendingPrepareRecovery = { target, identity: admissionSetIdentity(target), outcome: null, recovering: true };
  ...
  expect(internal.pendingPrepareRecovery).toBeNull();
...
test('scheduled recovery consumes async callback rejection and reuses its retry', async () => {
  const unhandled: unknown[] = [];
  let callbackCalls = 0;
  let finishRetry!: () => void;
  const retryCompleted = new Promise<void>((resolve) => { finishRetry = resolve; });
  const controller = new MasterIngressController({
    ...options(), leaseDurationMs: 1,
    onRecovered: async () => {
      callbackCalls += 1;
      if (callbackCalls === 1) throw new Error('recovery callback failed');
      finishRetry();
    },
  });
  ...
  process.on('unhandledRejection', onUnhandled);
  try {
    internal.scheduleRecovery();
    await waitWithTimeout(retryCompleted, 'recovery retry timed out');
    expect(callbackCalls).toBeGreaterThan(1);
    expect(unhandled).toEqual([]);
  } finally {
    process.off('unhandledRejection', onUnhandled);
    await controller.disconnect();
...
test('scheduled admission-resolution callback rejection retries without reviving stopped recovery', async () => {
  const unhandled: unknown[] = [];
  let callbackCalls = 0;
  let finishRetry!: () => void;
  const retryCompleted = new Promise<void>((resolve) => { finishRetry = resolve; });
  const controller = new MasterIngressController({
    ...
    onAdmissionResolved: async () => {
      callbackCalls += 1;
      if (callbackCalls === 1) throw new Error('resolution callback failed');
      finishRetry();
    },
  });
  const target = (controller as unknown as { toAdmissionSet(workers: readonly ServingConfigWorker[]): AdmissionSet }).toAdmissionSet([worker()]);
  attachFake(controller, {
    fence: async () => status({ active: target, prepared: null, retired: [] }),
    status: async () => status({ active: target, prepared: null, retired: [] }),
    lease: async () => status({ active: target, prepared: null, retired: [] }),
  });
  const internal = controller as unknown as {
    state: 'control_recovering';
    uncertainAdmission: unknown;
  ...
  try {
    internal.scheduleRecovery();
    await waitWithTimeout(retryCompleted, 'resolution retry timed out');
    expect(callbackCalls).toBeGreaterThan(1);
    expect(controller.currentState).toBe('attached');
    expect(unhandled).toEqual([]);
  } finally {
...
test('scheduled recovery does not restart after stop or disconnect', async () => {
  for (const action of ['stop', 'disconnect'] as const) {
    ...
    const callbackSettled = new Promise<void>((resolve) => { markSettled = resolve; });
    const callbackGate = new Promise<void>((resolve) => { releaseCallback = resolve; });
    const controller = new MasterIngressController({
      ...options(), leaseDurationMs: 1,
      onRecovered: () => {
        ...
        const callback = (async () => {
          await callbackGate;
          throw new Error('in-flight recovery callback failed');
        })();
        callback.then(markSettled, markSettled);
        return callback;
      },
    });
    attachFake(controller, { status: async () => status(), lease: async () => status() });
    const internal = controller as unknown as { state: 'control_recovering'; scheduleRecovery(): void; leaseTimer: ReturnType<typeof setTimeout> | null };
    internal.state = 'control_recovering';
    const unhandled: unknown[] = [];
    const onUnhandled = (reason: unknown): void => { unhandled.push(reason); };
    process.on('unhandledRejection', onUnhandled);
    try {
      internal.scheduleRecovery();
      await waitWithTimeout(callbackStarted, 'in-flight callback timed out');
      if (action === 'stop') controller.stopRecovery();
      else await controller.disconnect();
      releaseCallback();
      await waitWithTimeout(callbackSettled, 'callback completion timed out');
      await Promise.resolve();
      await Promise.resolve();
      expect(callbackCalls).toBe(1);
      expect(internal.leaseTimer).toBeNull();
      expect(unhandled).toEqual([]);
    } finally {
      releaseCallback();
      process.off('unhandledRejection', onUnhandled);
