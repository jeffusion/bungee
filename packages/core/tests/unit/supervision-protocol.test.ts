...
describe('strict supervision v1 protocol', () => {
  ...
  test('shutdown bypasses the command tail without bypassing replay protection', async () => {
    const authority = new SupervisionAuthorityGuard();
    establishAuthority(authority);
    const commands = new SupervisionCommandGuard(authority);
    let release!: () => void;
    const held = new Promise<void>((resolve) => { release = resolve; });
    const ordinary = commands.execute(signedCommand(2), 'ordinary', async () => { await held; return 'ordinary'; });
    let executions = 0;
    const shutdownMessage = signedCommand(3, OTHER_REQUEST, 1, CONTROLLER, '/shutdown');
    const shutdown = commands.executeShutdown(shutdownMessage, 'shutdown', () => { executions += 1; return 'shutdown'; });
    expect(commands.executeShutdown(shutdownMessage, 'different', () => 'not-run')).toBe(shutdown);
    expect(await shutdown).toEqual({ kind: 'settled', result_lookup_key: 'shutdown', result: 'shutdown' });
    expect(executions).toBe(1);
    expectCode(() => commands.execute(signedCommand(4, OTHER_REQUEST, 1, CONTROLLER, '/other'), 'conflict', () => 'not-run'), 'request_replay');
    release();
    await ordinary;
  });
});
